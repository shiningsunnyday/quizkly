...
class SparseAutoencoder(object):
    ...
    def outputgivenhidden(self, hidden):
        W2 = self.theta[self.limit1: self.limit2].reshape(self.visible_size, self.hidden_size)
        b2 = self.theta[self.limit3: self.limit4].reshape(self.visible_size, 1)
        return self.sigmoid(numpy.dot(W2, hidden) + b2)

    def hiddengiveninput(self, input):
        W1 = self.theta[self.limit0: self.limit1].reshape(self.hidden_size, self.visible_size)

        b1 = self.theta[self.limit2: self.limit3].reshape(self.hidden_size, 1)

        return self.sigmoid(numpy.dot(W1, input) + b1)
...
# def visualizeW1(opt_W1, vis_patch_side, hid_patch_side): TODO: No pyplot
#
